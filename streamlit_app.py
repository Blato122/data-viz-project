...
def create_interactive_map2():
    ...
    for i, (mun_geometry, (c, n)) in enumerate(zip(main_plot2.municipality_polygons, main_plot2.municipality_dict.items())): # aah ok
        color = main_plot2.color_mapping_interactive[(tuple(c), n)]
        style_func = StyleFunction(color)
        folium.GeoJson(
            mun_geometry,
            style_function=style_func,
            tooltip=folium.Tooltip(f"Gmina: {n}<br>Najbliższy szpital: {main_plot2.min_times_dict[(tuple(c), n)][0]}<br>Czas dojazdu: {main_plot2.min_times_dict[(tuple(c), n)][1]:.2f}")
        ).add_to(m)

    main_plot2.colormap.caption = 'Czas jazdy samochodem od centrum gminy do najbliższego szpitala (w minutach)'
    m.add_child(main_plot2.colormap)
    return m

# @st.cache_data # tutaj chyba lepiej trzymać jak najmniej rzeczy..?
def create_static_map2():
    # Plot the contours (filled with color) and health facility locations
    fig, ax = plt.subplots(figsize=(10, 10))
    main_plot2.municipalities.plot(ax=ax, edgecolor='grey', facecolor=[main_plot2.color_mapping[(tuple(c), n)] for c, n in main_plot2.municipality_dict.items()], linewidth=0.25)
    main_plot2.voivodeships.plot(ax=ax, edgecolor='black', facecolor="none", linewidth=0.25) # color czy facecolor? + alpha?
    # health_facilities[health_facilities['amenity'] == 'hospital'].plot(ax=ax, color='orange', markersize=10, label='Szpitale') # remove later? - some are points and some are exact polygons
    ...
    # Colormap legend as a bar - https://www.mimuw.edu.pl/~walen/vis/wybory2018/
    sm = plt.cm.ScalarMappable(cmap=main_plot2.cmap, norm=main_plot2.norm)
    cbar = plt.colorbar(
        sm,
        shrink=0.66,
        ax=plt.gca() # what does that do?
...
# @st.cache_data
def create_statistics_plot2():
    top_municipalities = [mun_name for (_, mun_name), _ in main_plot2.min_times_sorted[:20]]
    top_min_times_in_minutes = [time[1] for _, time in main_plot2.min_times_sorted[:20]]
    top_hospital_names = [time[0] for _, time in main_plot2.min_times_sorted[:20]]

    bottom_municipalities = [mun_name for (_, mun_name), _ in main_plot2.min_times_sorted[-20:]]
    bottom_min_times_in_minutes = [time[1] for _, time in main_plot2.min_times_sorted[-20:]]
    bottom_hospital_names = [time[0] for _, time in main_plot2.min_times_sorted[-20:]]
    ...
    top_20_most_often = {k: v for k, v in sorted(main_plot2.hospital_counts.items(), key=lambda c: c[1], reverse=True)[:20]} # refactor!!!!!! teraz na szybko
...
static2 = create_static_map2()
interactive2 = create_interactive_map2()
statistics2 = create_statistics_plot2()
